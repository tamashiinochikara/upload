...
def get_args_parser():
    ...
    parser.add_argument('--dataset_dir',default='G:\\cs\\code\\similarity\\train',
                        help='path where to load images')
    parser.add_argument('--test_image_dir',default='G:\\cs\\code\\similarity\\val',
    ...
    parser.add_argument('--model_name',default='resnet50',help='model name')#resnet50 resnet152 clip
    parser.add_argument('--feature_dict_file',default='corpus_feature_dict.npy',
                        help='filename where to save image representations')#存储数据库中图像特征的字典
    parser.add_argument('--topk',default=7,type=int,help='k most similar images to be picked')

    parser.add_argument('--mode',default='extract',#extract or preditct
...
def extract_feature_single(args,model,file):
    ...
    trainset_mean = [0.47230713,0.43488868, 0.32462729]
    trainset_std = [0.37648914, 0.35993242, 0.34772052]
...
def extract_feature_by_CLIP(model, preproces ,file):
    image = preproces(Image.open(file)).unsqueeze(0).to(device)
    with torch.no_grad():
        vec = model.encode_image(image)
        vec = vec.squeeze().cpu().numpy()
    return vec
def extract_features(args,model, image_path='',preprocess=None):#image_path 对应train文件夹
    allVectors = {}
    ...
    os.makedirs(f'{args.save_dir}\\{args.model_name}',exist_ok=True)#创建了resnet50
...
def getSimilarityMatrix(vectors_dict):
    ...
    v = np.array(list(vectors_dict.values()))#[NUM,H]数量和大小
    numerator = np.matmul(v,v.T) #[NUM,NUM]内积
    denominator = np.matmul(np.linalg.norm(v,axis=1, keepdims=True), np.linalg.norm(v,axis=1, keepdims=True).T)#[NUM,NUM]模
...
if __name__ == '__main__':
    ...
    if args.mode == "extract":#第一阶段 表征提取
        ...
        allVecotor = np.load(f'{args.save_dir}\\{args.model_name}\\{args.feature_dict_file}',allow_pickle=True)
        allVecotor = allVecotor.item()

        # reading test images
        for image_file in tqdm.tqdm(test_images):#测试照片的特征
            print(f'reading {image_file}...')

            if args.model_name == 'clip':
                #CLIP model
                allVecotor[image_file] = extract_feature_by_CLIP(model, preprocess, image_file)
            else:
                #resnet50/152 model
                allVecotor[image_file] = extract_feature_single(args,model,image_file)
        sim,keys = getSimilarityMatrix(allVecotor)

        result = {}

        for image_file in tqdm.tqdm(test_images):
            print(f'sorting most similar images aas {image_file}...')

            index = keys.index(image_file)
            sim_vec = sim[index]

            indexs = np.argsort(sim_vec)[::-1][1:args.topk]#从小到大 并过滤本身
